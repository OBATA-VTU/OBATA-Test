...
export const TransactionTerminal: React.FC = () => {
  ...
  const syncServices = async () => {
    setIsSyncing(true);
    setRawResponse(null);
    const tid = toast.loading("Connecting to Node Gateway...");
    
    try {
      // 1. Fetch catalog from the backend API
      const res = await fetch('/api/terminal/services');
      const apiData = await res.json();
      
      if (apiData.status !== 'success') {
        setRawResponse({ step: "API_FETCH_FAILED", data: apiData });
        throw new Error(`API Hub Refused Request: ${apiData.message || 'Unknown'}`);
      }

      setRawResponse({ 
          step: "API_DATA_RECEIVED", 
          message: "Handshake verified. Processing batch injection...", 
          count: {
      ...
      data.airtime?.forEach((item: any) => {
          ...
          batch.set(ref, { ...item, type: 'AIRTIME', label: `${item.network} Airtime`, lastSync: new Date().toISOString() });
      });

      // Data Plans Mapping
      data.dataPlans?.forEach((item: any) => {
          const ref = doc(db, 'synced_services', `DATA_${item.serviceID}`);
          batch.set(ref, { ...item, type: 'DATA', label: `${item.network} ${item.dataPlan}`, lastSync: new Date().toISOString() });
      });

      // Cable Mapping
      data.cablePlans?.forEach((item: any) => {
          const ref = doc(db, 'synced_services', `CABLE_${item.serviceID}`);
          batch.set(ref, { ...item, type: 'CABLE', label: `${item.cable} - ${item.cablePlan}`, lastSync: new Date().toISOString() });
      });

      // Power Mapping
      data.electricity?.forEach((item: any) => {
          const ref = doc(db, 'synced_services', `POWER_${item.serviceID}`);
          batch.set(ref, { ...item, type: 'POWER', label: item.disco, lastSync: new Date().toISOString() });
      });

      // FIREBASE COMMIT
      try {
          await batch.commit();
          toast.success("Intelligence Grid Synced!", { id: tid });
          setRawResponse({ status: "SUCCESS", message: "Firebase successfully populated with service catalog." });
          loadLocalServices();
      } catch (fbErr: any) {
          console.error("FIREBASE ERROR:", fbErr);
          setRawResponse({
              status: "FIREBASE_BLOCK",
              code: fbErr.code,
              message: fbErr.message,
              hint: "If this persists, check if the Firestore database has been created in the Firebase console."
          });
          throw new Error(`Cloud Write Rejected: ${fbErr.code}`);
      }
  ...
  const handleVerify = async () => {
    ...
    setRawResponse({ action: "VERIFY_NODE", category, serviceID: form.serviceID });
    ...
    try {
        ...
        if (customer) {
            ...
            toast.success(`Identity: ${customer}`);
        } else {
  ...
  const handlePurchase = async () => {
    setIsLoading(true);
    setRawResponse(null);
    const tid = toast.loading("Dispatching Packet...");
    try {
        let endpoint = '/api/terminal/purchase/';
        let payload: any = { serviceID: form.serviceID, requestId: `LAB-${Date.now()}` };
        ...
        if (data.status === 'success' || data.success) {
            toast.success("Packet Acknowledged", { id: tid });
            fetchStatus();
        } else {
            toast.error(data.message || "Command Rejected", { id: tid });
        }
    } catch (e) {
        toast.error("Uplink Lost", { id: tid });
    } finally {
        setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      {/* Header Diagnostic UI */}
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-slate-900 pb-8 text-left">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.4em] mt-1">Cross-Origin API & Cloud Storage Bridge</p>
            </div>
        </div>
        ...
        <div className="flex bg-slate-900 p-1 rounded-2xl border border-slate-800 shadow-xl">
           ...
           <button onClick={() => setActiveTab('PAYSTACK')} className={`px-6 py-2.5 rounded-xl text-[9px] font-black transition-all ${activeTab === 'PAYSTACK' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white'}`}>DEBUG_INFO</button>
        </div>
      </div>
            ...
            {activeTab === 'PURCHASE_LAB' && (
                <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="space-y-8 relative z-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-6">
                                {(category === 'AIRTIME' || category === 'DATA') && (
                                    <div>
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Carrier Protocol</label>
                                ...
                                <div>
                                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Service Node (Cloud ID)</label>
                                    <select 
                                ...
                                {(category === 'AIRTIME' || category === 'DATA') && (
                                    <div>
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Terminal Number</label>
                                        <input 
                                ...
                                {(category === 'AIRTIME' || category === 'POWER') && (
                                    <div>
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Value Packet (₦)</label>
                                        <input 
                                ...
                                {(category === 'CABLE' || category === 'POWER') && (
                                    <div className="space-y-6">
                                        <div>
                                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Account Reference (IUC/Meter)</label>
                                            <div className="flex gap-2">
                                                ...
                                                <button disabled={isValidating || (!form.iucNum && !form.meterNum) || !form.serviceID} onClick={handleVerify} className="bg-slate-800 hover:bg-white hover:text-black px-6 rounded-2xl text-[8px] font-black uppercase transition-all disabled:opacity-30 flex items-center shadow-lg">
                                                    {isValidating ? <RefreshCw className="w-4 h-4 animate-spin" /> : 'VERIFY_ACCOUNT'}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="flex flex-col justify-center items-center text-center p-10 bg-slate-950/50 rounded-[3rem] border-2 border-slate-800 border-dashed relative overflow-hidden group/probe">
                                 ...
                                 {validatedName ? (
                                    <div className="space-y-4 animate-fade-in">
                                        ...
                                        <div>
                                            <p className="text-[8px] font-black text-slate-500 uppercase tracking-[0.4em] mb-1">Identity Verified</p>
                        ...
                        >
                            {isLoading ? <RefreshCw className="w-6 h-6 animate-spin" /> : <><Play className="w-5 h-5 mr-4" /> DISPATCH_COMMAND</>}
                        </button>
                    </div>
                </div>
            )}
                    ...
                    <div className="flex justify-between items-end mb-10 border-b border-slate-800 pb-8">
                        <div>
                            <h3 className="text-3xl font-black text-white flex items-center uppercase tracking-tighter">
                                <Database className="w-8 h-8 mr-3 text-blue-500" /> Cloud Registry
                            </h3>
                            <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mt-2 ml-1">Migrate API catalog to Firebase storage</p>
                        </div>
                        <button 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto no-scrollbar pr-2">
                        {syncedServices.length === 0 ? (
                            <div className="col-span-2 py-20 text-center text-slate-800 flex flex-col items-center gap-4">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-widest">Firestore Hub Offline (No data)</p>
                            </div>
                        ) : (
            ...
            {activeTab === 'PAYSTACK' && (
                <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-12 shadow-2xl animate-fade-in relative overflow-hidden">
                    <h3 className="text-3xl font-black text-white mb-8 flex items-center uppercase tracking-tighter">
                        <Monitor className="w-8 h-8 mr-3 text-emerald-500" /> Logic Dissection
                    </h3>
                    <p className="text-slate-400 text-sm mb-10 leading-relaxed font-medium">
                        Permissions are now fully relaxed for service catalogs. If the "Sync" fails, ensure your Firebase Firestore instance is initialized in the Google Console. The batch commit requires an active database to accept the incoming API packets.
                    </p>
                    <div className="bg-black/50 p-8 rounded-[2rem] border border-slate-800 font-mono text-[10px] uppercase tracking-[0.2em] leading-loose text-emerald-500 shadow-inner">
                        ...
                        &gt; FIREBASE_PERMISSION: BYPASSED<br/>
                        &gt; BATCH_OPERATION: READY<br/>
                        &gt; API_PROXY: ONLINE
                    </div>
                </div>
            )}
            ...
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] relative overflow-hidden group">
                <div className="relative z-10">
                    <div className="flex items-center space-x-3 mb-4">
                        ...
                        <h4 className="text-white font-black text-[9px] uppercase tracking-widest">Handshake Verified</h4>
                    </div>
                    <p className="text-slate-500 text-[10px] font-bold leading-relaxed uppercase tracking-tighter italic opacity-60">
                        Cross-origin requests are bridged via production edge proxies. Data validation is performed at the node level.
                    </p>
                </div>
            </div>
        </div>
