...
export const TransactionTerminal: React.FC = () => {
  ...
  const syncServices = async () => {
    setIsSyncing(true);
    setRawResponse(null);
    const tid = toast.loading("Connecting to Inlomax Node...");
    ...
    try {
      ...
      setRawResponse({ 
          ...
          message: "Uplink verified. Commencing Firebase Injection...", 
          count: {
      ...
      try {
          ...
          setRawResponse({ status: "SUCCESS", message: "All nodes successfully registered in Cloud Firestore." });
          loadLocalServices();
      } catch (fbErr: any) {
          ...
          const report = {
              ...
              hint: "Ensure Firestore Rules are deployed and you are logged in."
          };
          setRawResponse(report);
          throw new Error(`Cloud Access Denied: ${fbErr.code}`);
      }
  ...
  const handleVerify = async () => {
    ...
    setRawResponse({ action: "NODE_VALIDATION_INIT", payload: { category, id: form.serviceID } });
    ...
    try {
        ...
        const customer = data.customerName || data.data?.customerName;
        if (customer) {
  ...
  const handlePurchase = async () => {
    setIsLoading(true);
    setRawResponse(null);
    const tid = toast.loading("Dispatched Command...");
    try {
        ...
        if (data.status === 'success') {
            toast.success("Transaction Complete", { id: tid });
            fetchStatus();
        } else {
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-slate-900 pb-8">
        <div className="flex items-center gap-4">
            ...
            <div className="text-left">
                <h1 className="text-3xl font-black text-white tracking-tighter uppercase italic">PROTOCOL_<span className="text-blue-500">X</span></h1>
                <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.4em] mt-1 ml-0.5">Firebase & API Bridge Lab</p>
            </div>
        </div>
                    ...
                    <div className="space-y-8 relative z-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-6">
                                ...
                                <div>
                                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Active Nodes (ID)</label>
                                    <select 
                                ...
                                {(category === 'AIRTIME' || category === 'DATA') && (
                                    <div>
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Recipient Terminal</label>
                                        <input 
                                ...
                                {(category === 'AIRTIME' || category === 'POWER') && (
                                    <div>
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Value Injection (₦)</label>
                                        <input 
                                ...
                                {(category === 'CABLE' || category === 'POWER') && (
                                    <div className="space-y-6">
                                        <div>
                                            <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Serial ID (IUC/Meter)</label>
                                            <div className="flex gap-2">
                                                ...
                                                <button disabled={isValidating || (!form.iucNum && !form.meterNum) || !form.serviceID} onClick={handleVerify} className="bg-slate-800 hover:bg-white hover:text-black px-6 rounded-2xl text-[8px] font-black uppercase transition-all disabled:opacity-30 flex items-center shadow-lg">
                                                    {isValidating ? <RefreshCw className="w-4 h-4 animate-spin" /> : 'VERIFY_SERIAL'}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="flex flex-col justify-center items-center text-center p-10 bg-slate-950/50 rounded-[3rem] border-2 border-slate-800 border-dashed relative overflow-hidden group/probe">
                                 ...
                                 {validatedName ? (
                                    ...
                                    <div className="opacity-10 space-y-4">
                                        ...
                                        <p className="text-[8px] font-black uppercase tracking-[0.6em]">Node Standby</p>
                                    </div>
                                 )}
                            </div>
                        </div>
                    ...
                    <div className="flex justify-between items-end mb-10 border-b border-slate-800 pb-8">
                        <div className="text-left">
                            <h3 className="text-3xl font-black text-white flex items-center uppercase tracking-tighter">
                                <Database className="w-8 h-8 mr-3 text-blue-500" /> Registry Sync
                            </h3>
                            <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mt-2 ml-1">Clone API Catalog to Cloud Firestore</p>
                        </div>
                        <button 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto no-scrollbar pr-2">
                        {syncedServices.length === 0 ? (
                            <div className="col-span-2 py-20 text-center text-slate-800 flex flex-col items-center gap-4">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-widest">Cloud Registry Offline</p>
                            </div>
                        ) : (
            ...
            {activeTab === 'PAYSTACK' && (
                <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-12 shadow-2xl animate-fade-in relative overflow-hidden">
                    <h3 className="text-3xl font-black text-white mb-8 flex items-center uppercase tracking-tighter">
                        <Monitor className="w-8 h-8 mr-3 text-emerald-500" /> Cloud Log Debug
                    </h3>
                    <p className="text-slate-400 text-sm mb-10 leading-relaxed font-medium text-left">
                        This terminal routes all requests through the Vercel Proxy. The "Sync" command fetches the entire Inlomax catalog and pushes it to Firebase. If you see errors in the "Trace Log", it means Firestore rejected the write. Ensure the Rules I provided are deployed in your Firebase Console.
                    </p>
                    <div className="bg-black/50 p-8 rounded-[2rem] border border-slate-800 font-mono text-[10px] uppercase tracking-[0.2em] leading-loose text-emerald-500 shadow-inner text-left">
                        ...
                        &gt; FIREBASE_COMMITS: READY<br/>
                        &gt; PERMISSION_BYPASS: ACTIVE<br/>
                        &gt; GATEWAY_UPLINK: STABLE
                    </div>
                </div>
            )}
            ...
            <div className="bg-black border border-slate-800 rounded-[3rem] overflow-hidden flex flex-col flex-1 shadow-2xl relative min-h-[500px]">
                <div className="p-8 border-b border-slate-900 flex justify-between items-center bg-slate-950/50 relative z-10">
                    <h3 className="text-[9px] font-black text-white uppercase tracking-[0.4em] flex items-center">
                        <Terminal className="w-4 h-4 mr-3 text-blue-500" /> Trace_Log
                    </h3>
                ...
                <div className="flex-1 p-8 overflow-auto font-mono text-[11px] no-scrollbar custom-scroll text-left relative z-10">
                    {!rawResponse ? (
            ...
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] relative overflow-hidden group">
                <div className="relative z-10 text-left">
                    ...
                    <p className="text-slate-500 text-[10px] font-bold leading-relaxed uppercase tracking-tighter italic opacity-60">
                        External requests bridged via Vercel Edge. Firebase interactions are direct-node commits.
                    </p>
                </div>
            </div>
        </div>
