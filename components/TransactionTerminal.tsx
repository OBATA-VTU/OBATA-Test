...
export const TransactionTerminal: React.FC = () => {
  ...
  const syncServices = async () => {
    ...
    try {
      ...
      await batch.commit();
      toast.success("Catalog Synchronized!", { id: tid });
      setRawResponse(apiData);
      loadLocalServices();
    } catch (e: any) {
  ...
  const handleVerify = async () => {
    ...
    try {
        ...
        if (data.status === 'success' || data.customerName || data.data?.customerName) {
            ...
            toast.success(`Matched: ${name}`, { id: tid });
        } else {
  ...
  const handlePurchase = async () => {
    setIsLoading(true);
    setRawResponse(null);
    const tid = toast.loading("Processing Purchase Node...");
    try {
    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 space-y-8">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 shadow-2xl animate-fade-in relative overflow-hidden">
                    <div className="flex items-center justify-between mb-10 overflow-x-auto no-scrollbar gap-4 border-b border-slate-800 pb-8">
                        {[
                            ...
                            <button 
                                key={cat.id} 
                                onClick={() => { setCategory(cat.id as PurchaseCategory); setValidatedName(null); }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-4 gap-2">
                                        {['MTN', 'AIRTEL', 'GLO', '9MOBILE'].map(net => (
                                            <button 
                                                ...
                                                className={`py-3 rounded-xl text-[9px] font-black border transition-all ${selectedNetwork === net ? 'bg-white text-black border-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}
                                            >
                                                {net}
                                            </button>
                                        ))}
                            ...
                            <div>
                                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Service Descriptor</label>
                                <select 
                            ...
                            {(category === 'AIRTIME' || category === 'DATA') && (
                                <div>
                                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Target Number</label>
                                    <input 
                                        type="text" value={form.mobileNumber} 
                                        onChange={e => setForm({...form, mobileNumber: e.target.value.replace(/\D/g, '')})}
                                        className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-5 text-white font-mono text-lg tracking-widest outline-none focus:border-blue-500"
                            ...
                            {category === 'AIRTIME' && (
                                <div>
                                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Purchase Amount</label>
                                    <input 
                                        type="number" value={form.amount} 
                                        onChange={e => setForm({...form, amount: e.target.value})}
                                        className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-5 text-white font-black text-xl outline-none focus:border-blue-500"
                                        placeholder="100"
                                    />
                                </div>
                            )}
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <input 
                                            ...
                                            className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl p-5 text-white font-mono tracking-widest outline-none focus:border-blue-500"
                                            placeholder="1029384756"
                                        />
                                        <button onClick={handleVerify} className="bg-slate-800 hover:bg-white hover:text-black px-6 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all">Verify</button>
                                    </div>
                                </div>
                            )}
                            ...
                            {category === 'POWER' && (
                                <div className="space-y-6">
                                    <div className="grid grid-cols-2 gap-4">
                                        <button onClick={() => setForm({...form, meterType: '1'})} className={`py-4 rounded-xl text-[9px] font-black border transition-all ${form.meterType === '1' ? 'bg-amber-600 border-amber-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>PREPAID</button>
                                        <button onClick={() => setForm({...form, meterType: '2'})} className={`py-4 rounded-xl text-[9px] font-black border transition-all ${form.meterType === '2' ? 'bg-amber-600 border-amber-600 text-white' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>POSTPAID</button>
                                    </div>
                                    <div>
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Meter Number</label>
                                        <div className="flex gap-2">
                                            <input 
                                                type="text" value={form.meterNum} 
                                                onChange={e => setForm({...form, meterNum: e.target.value.replace(/\D/g, '')})}
                                                className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl p-5 text-white font-mono tracking-widest outline-none focus:border-amber-500"
                                    ...
                                    <div>
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Payable Amount</label>
                                        <input 
                                            type="number" value={form.amount} 
                                            onChange={e => setForm({...form, amount: e.target.value})}
                                            className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-5 text-white font-black text-xl outline-none focus:border-amber-500"
                                            placeholder="1000"
                                        />
                                    </div>
                                </div>
                            ...
                            {category === 'EDUCATION' && (
                                <div>
                                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 block">Token Quantity</label>
                                    <input 
                        ...
                        <div className="flex flex-col justify-center items-center text-center p-10 bg-slate-950/50 rounded-[2.5rem] border border-slate-800 border-dashed">
                             {validatedName ? (
                                <div className="space-y-4 animate-fade-in">
                                    <div className="bg-emerald-500/10 p-5 rounded-full mx-auto w-fit border border-emerald-500/20"><UserCheck className="w-10 h-10 text-emerald-500" /></div>
                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Ownership Verified</p>
                                    <h4 className="text-xl font-black text-white">{validatedName}</h4>
                                </div>
                             ) : (
                                <div className="opacity-20 space-y-4">
                                    <ShieldCheck className="w-16 h-16 mx-auto" />
                                    <p className="text-[9px] font-black uppercase tracking-[0.3em]">Awaiting Identity Pulse</p>
                                </div>
                             )}
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full mt-10 bg-blue-600 hover:bg-blue-500 text-white font-black py-7 rounded-[2rem] transition-all shadow-2xl shadow-blue-600/20 flex items-center justify-center active:scale-95 disabled:opacity-50"
                    >
                        {isLoading ? <RefreshCw className="w-6 h-6 animate-spin" /> : <><Play className="w-6 h-6 mr-3" /> INITIALIZE TRANSACTION</>}
                    </button>
                </div>
            )}
                    ...
                    <div className="relative z-10">
                        <div className="flex justify-between items-center mb-10">
                            ...
                            <button 
                                ...
                                disabled={isSyncing}
                                className="bg-purple-600 hover:bg-purple-500 text-white px-8 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all flex items-center shadow-xl shadow-purple-600/20"
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[500px] overflow-y-auto pr-2 custom-scroll">
                            {syncedServices.map(svc => (
                                <div key={svc.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl flex items-center space-x-4 hover:border-purple-500/30 transition-all group">
            ...
            {/* Paystack implementation remains static for identification checks */}
            {activeTab === 'PAYSTACK' && (
                <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-10 shadow-2xl animate-fade-in">
                    <h3 className="text-xl font-black text-white mb-8 flex items-center uppercase tracking-tighter">
                        <Code className="w-6 h-6 mr-3 text-emerald-500" /> External Node Relay
                    </h3>
                    <p className="text-slate-500 text-sm mb-10 leading-relaxed">System is configured to bridge Paystack bank resolution nodes via the production proxy. Ensure the <code>PAYSTACK_SECRET_KEY</code> is correctly set in your Vercel Environment Variables.</p>
                </div>
            )}
