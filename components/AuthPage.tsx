...
export const AuthPage: React.FC<AuthPageProps> = ({ onSuccess, initialMode = 'LOGIN' }) => {
  ...
  const generateApiKey = () => {
    return `OBATA_${crypto.randomUUID()}`;
  };
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!existingDoc.exists()) {
         ...
         const newUser: Omit<UserProfile, 'uid'> & { uid: string } = {
            uid: user.uid,
            email: user.email,
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (usernameTaken) {
            throw new Error(`Username '${regUsername}' is already taken.`);
        }

        const userCredential = await createUserWithEmailAndPassword(auth, regEmail, regPassword);
        const user = userCredential.user;

        // Create document strictly matching the user's schema
        const newUser: Omit<UserProfile, 'uid'> & { uid: string, pendingUpgrade?: boolean } = {
            uid: user.uid,
            email: regEmail,
            username: regUsername,
            isReseller: false, // Always false initially
            walletBalance: 0,
            referralCode: generateReferralCode(),
            referredBy: referralCode || null,
            createdAt: serverTimestamp(),
            apiKey: generateApiKey(),
            photoURL: "https://i.ibb.co/9HLqrnZG/7c973cd9-cee7-40ed-af89-fd5dc5b5339e670713365240472949.jpg", // Default avatar
            transactionPin: "0000",
            emailNotificationsEnabled: true,
            hasFunded: false,
            hasMadePurchase: false,
            totalPurchaseValue: 0,
            pendingUpgrade: wantReseller // Temporary flag for UI to prompt payment
        };
         ...
         {mode === 'LOGIN' ? (
             ...
             <form onSubmit={handleRegister} className="space-y-4">
                 ...
                 <div className="grid grid-cols-2 gap-3 pt-2">
                     <div 
                        onClick={() => setWantReseller(false)}
                        className={`cursor-pointer border rounded-xl p-3 flex flex-col items-center text-center transition-all ${!wantReseller ? 'bg-blue-600/10 border-blue-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-600'}`}
                     >
                         <Shield className={`w-6 h-6 mb-2 ${!wantReseller ? 'text-blue-500' : 'text-slate-500'}`} />
                         <span className="text-xs font-bold">Smart Earner</span>
                         <span className="text-[10px] text-slate-500">Free Account</span>
                     </div>
                     <div 
                        onClick={() => setWantReseller(true)}
                        className={`cursor-pointer border rounded-xl p-3 flex flex-col items-center text-center transition-all ${wantReseller ? 'bg-amber-600/10 border-amber-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-600'}`}
                     >
                         <Shield className={`w-6 h-6 mb-2 ${wantReseller ? 'text-amber-500' : 'text-slate-500'}`} />
         ...
         <div className="mt-6 pt-6 border-t border-slate-800">
             ...
             >
                 <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
                    ...
                    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.47 1.18 4.93l2.85-2.84z" fill="#FBBC05"/>
